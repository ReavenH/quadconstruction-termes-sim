...
# 3. 获取所有 Layers 和源文件
layers = np.sort(combined_data["Layer"].unique())[:3]  # 正序排列 Layer
...
for i, layer in enumerate(layers):
    ...
    for source in sources:
        ...
        means = source_data["mean_overlapAreaPercentage"].values
        stds = source_data["std_overlapAreaPercentage"].values
    ...
    ax.axhline(y=0.75, color='gray', linestyle='--', linewidth=1.75)
    # 子图设置
...
fig.suptitle("Overlap Proportion v.s. Expansion of Layer\n(Selected Layers)", fontsize = 16)
# 调整布局以确保图例显示
